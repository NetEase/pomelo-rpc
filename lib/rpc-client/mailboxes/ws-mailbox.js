...
pro.connect = function(tracer, cb) {
  tracer.info('pomelo-rpc', true, this.id, __filename, 'connect', 'ws-mailbox try to connect');
  if(this.connected) {
...
pro.send = function(tracer, msg, opts, cb) {
  tracer.info('pomelo-rpc', true, this.id, __filename, 'send', 'ws-mailbox try to send');
  if(!this.connected) {
