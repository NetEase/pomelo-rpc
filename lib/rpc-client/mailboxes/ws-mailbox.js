...
pro.connect = function(tracer, cb) {
  tracer.info('pomelo-rpc', true, this.id, __filename, 'connect', {host: this.host, port: this.port}, 'ws-mailbox try to connect');
  if(this.connected) {
...
pro.send = function(tracer, msg, opts, cb) {
  tracer.info('pomelo-rpc', true, this.id, __filename, 'send', msg, 'ws-mailbox try to send');
  if(!this.connected) {
  ...
  var id = this.curId++;
  this.requests[id] = cb;
  var pkg = {tracer: tracer, id: id, msg: msg};
...
var processMsg = function(mailbox, pkg) {
  ...
  var tracer = new Tracer(mailbox.opts.rpcLogger, mailbox.opts.rpcDebugLog, pkg.tracer.id, pkg.tracer.seq);
  var args = [tracer];
