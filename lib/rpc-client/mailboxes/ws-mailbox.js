...
pro.connect = function(traceId, cb) {
  this.loggerUtil.info(traceId, 'pomelo-rpc', true, this.id, __filename, 'connect', 'ws-mailbox try to connect');
  if(this.connected) {
  ...
  this.socket.on('message', function(pkg) {
    try {
      ...
      // socket.io would broke if not catch the exception
    }
  });
 ...
 */
pro.send = function(traceId, msg, opts, cb) {
  this.loggerUtil.info(traceId, 'pomelo-rpc', true, this.id, __filename, 'send', 'ws-mailbox try to send');
  if(!this.connected) {
  ...
  var id = this.curId++;
  this.requests[id] = cb;
  var pkg = {traceId: traceId, id: id, msg: msg};
