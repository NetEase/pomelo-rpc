...
pro.connect = function(traceId, cb) {
  this.loggerUtil.info(traceId, 'pomelo-rpc', true, this.id, __filename, 'connect', 'tcp-mailbox try to connect');
  if(this.connected) {
...
pro.send = function(traceId, msg, opts, cb) {
  this.loggerUtil.info(traceId, 'pomelo-rpc', true, this.id, __filename, 'send', 'tcp-mailbox try to send');
  if(!this.connected) {
