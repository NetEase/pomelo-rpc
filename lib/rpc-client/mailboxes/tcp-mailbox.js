...
pro.connect = function(tracer, cb) {
  tracer.info(tracer.id, 'pomelo-rpc', true, this.id, __filename, 'connect', 'tcp-mailbox try to connect');
  if(this.connected) {
...
pro.send = function(tracer, msg, opts, cb) {
  tracer.info(tracer.id, 'pomelo-rpc', true, this.id, __filename, 'send', 'tcp-mailbox try to send');
  if(!this.connected) {
