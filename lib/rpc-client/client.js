...
var failCluster = function(client, tracer, serverId, msg, cb) {
  var servers, type;
  var server = client._station.servers[serverId] || client._routeContext.getServerFromConfig(serverId);
  if(!!server) {
    ...
    throw new Error('[pomelo-rpc] could not find rpc target server ' + serverId);
    return;
  }
  var handle = function(id, callback) {
    client._station.dispatch(tracer, id, msg, client.opts, function() {
      var args = Array.prototype.slice.call(arguments, 0);
      callback(args);
    });
  };

  var domainHandle = function(domain, id, callback) {
    domain.run(function() {
      client._station.dispatch(tracer, id, msg, client.opts, function() {
        var args = Array.prototype.slice.call(arguments, 0);
        callback(args);
    ...
    case 'failover':
      ...
      d.on('error', function(err) {
        ...
        if(counter < retries + 1) {
          domainHandle(d, servers[counter], function(args) {
