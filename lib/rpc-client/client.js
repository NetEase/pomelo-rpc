...
pro.rpcInvoke = function(serverId, msg, cb) {
  ...
  rpcModeProcess(this, tracer, serverId, msg, this.opts, cb);
};
...
var proxyCB = function(client, serviceName, methodName, args, attach, invoke, isToSpecifiedServer) {
  ...
  if (isToSpecifiedServer) {
    rpcToSpecifiedServer(client, msg, serverType, routeParam);
  } else {
    getRouteTarget(client, serverType, routeParam, function(err, serverId) {
      console.error('serverId : %s', serverId);
      if(!!err) {
        utils.invokeCallback(cb, err);
      } else {
...
var getRouteTarget = function(client, serverType, routeParam, cb) {
  if(!!client.routerType) {
    ...
    method.call(null, client, serverType, function(err, serverId) {
...
var rpcToSpecifiedServer = function(client, msg, serverType, routeParam) {
  ...
  if (routeParam === '*') {
    ...
    if (!server || server.serverType !== serverType) {
...
var rpcModeProcess = function(client, tracer, serverId, msg, opts, cb) {
  var method;
  var mode = opts.failMode;
  switch(mode) {
    case constants.FAIL_MODE.FAILOVER:
      method = failureProcess.failover;
    break;
    case constants.FAIL_MODE.FAILBACK:
      method = failureProcess.failback;
    break;
    case constants.FAIL_MODE.FAILFAST:
      method = failureProcess.failfast;
    break;
    case constants.FAIL_MODE.FAILSAFE:
    default:
      method = failureProcess.failsafe;
    break;
  }
  method.call(null, client, tracer, serverId, msg, opts, cb);
};
