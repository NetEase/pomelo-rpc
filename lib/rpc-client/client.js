...
pro.rpcInvoke = function(serverId, msg, cb) {
  ...
  failCluster(this, tracer, serverId, msg, cb);
};
...
var failCluster = function(client, tracer, serverId, msg, cb) {
  var servers, type;
  var server = client._station.servers[serverId];
  if(!!server) {
    type = server.serverType;
    servers = client._station.serversMap[type];
  ...
  var handle = function(id, callback) {
    client._station.dispatch(tracer, id, msg, null, function() {
      var args = Array.prototype.slice.call(arguments, 0);
      callback(args);
    });
  };

  var mode = client.opts.failMode;
  switch(mode) {
    case 'failover':
      var counter = 0;
      var flag = true;
      var rs = null;
      var retries = client.opts.retries || 2;
      if(servers.length < retries) {
        logger.warn('the rpc retries option configure with error, for it is larger than the servers length');
        retries = servers.length;
      }
      servers.splice(0, 0, serverId);
      async.whilst(
        function() {
          return counter < retries + 1 && flag;
        },
        function(callback) {
          handle(servers[counter], function(args) {
            if(utils.getObjectClass(args[0]) === 'Error') {
              counter++;
              if(!rs) {
                rs = args;
              }
        ...
        function(err) {
          if(counter === retries + 1 && flag) {
            utils.applyCallback(cb, rs);
          }
        }
      );
    break;
