...
var MailStation = function(opts) {
  ...
  this.retryConnectTime = opts.retryConnectTime || 5 * 1000;
...
pro.dispatch = function(tracer, serverId, msg, opts, cb) {
  ...
  if(!mailbox) {
    ...
    if(!lazyConnect(tracer, this, serverId, this.mailboxFactory, cb)) {
      logger.error('lazyConnect failed to remote server : ' + serverId);
...
var lazyConnect = function(tracer, station, serverId, factory, cb) {
  ...
  connect(tracer, mailbox, station, serverId, cb);
  return true;
};

var connect = function(tracer, mailbox, station, serverId, cb) {
  mailbox.connect(tracer, function(err) {
    if(err) {
      logger.error('[pomelo-rpc] mailbox fail to connect to remote server: %s, err: %j', serverId, err.stack);
      reconnect(tracer, station, serverId, cb);
      return;
    }
...
var reconnect = function(tracer, station, serverId, cb) {
  ...
  if(mailbox.retry > station.retryTimes && !!station.mailboxes[serverId]) {
    delete station.mailboxes[serverId];
    utils.invokeCallback(cb, new Error('connect to remote server failed.'));
    return;
  }
  setTimeout(function() {
    connect(tracer, mailbox, station, serverId, cb);
  }, station.retryConnectTime * mailbox.retry);
};
...
var addToPending = function(tracer, station, serverId, args) {
  ...
  if(pending.length > station.pendingSize) {
    logger.error('[pomelo-rpc] station pending too much for: %s',  serverId);
    return;
  }
  pending.push(args);
};
