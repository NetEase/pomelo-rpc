...
pro.dispatch = function(tracer, serverId, msg, opts, cb) {
  tracer.info('pomelo-rpc', true, serverId, __filename, 'dispatch', msg, 'dispatch rpc message to the mailbox');
  if(this.state !== STATE_STARTED) {
  ...
  var send = function(tracer, serverId, msg, opts) {
    tracer.info('pomelo-rpc', true, serverId, __filename, 'send', msg, 'get corresponding mailbox and try to send message');
...
var doFilter = function(tracer, serverId, msg, opts, filters, index, operate, cb) {
  tracer.info('pomelo-rpc', true, serverId, __filename, 'doFilter', msg, 'do ' + operate + ' filter ' + index);
...
var lazyConnect = function(tracer, station, serverId, factory) {
  tracer.info('pomelo-rpc', true, serverId, __filename, 'lazyConnect', null, 'create mailbox and try to connect to remote server');
...
var addToPending = function(tracer, station, serverId, args) {
  tracer.info('pomelo-rpc', true, serverId, __filename, 'addToPending', null, 'add pending requests to pending queue');
...
var flushPending = function(tracer, station, serverId) {
  tracer.info('pomelo-rpc', true, serverId, __filename, 'flushPending', null, 'flush pending requests to dispatch method');
  var pending = station.pendings[serverId];
