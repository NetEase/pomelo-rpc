...
var MailStation = function(opts) {
  ...
  this.loggerUtil = new LogUtil(opts.rpcLogger, opts.rpcDebugLog);
 ...
 * @return {Void}
 */
pro.dispatch = function(traceId, serverId, msg, opts, cb) {
  this.loggerUtil.info(traceId, 'pomelo-rpc', true, serverId, __filename, 'dispatch', 'dispatch rpc message to the mailbox');
  if(this.state !== STATE_STARTED) {
  ...
  if(!mailbox) {
    ...
    if(!lazyConnect(traceId, this, serverId, this.mailboxFactory)) {
      utils.invokeCallback(cb, new Error('fail to connect to remote server:' + serverId));
      return;
    }
    // push request to the pending queue
    addToPending(traceId, this, serverId, Array.prototype.slice.call(arguments, 0));
    return;
  }
  ...
  var send = function(traceId, serverId, msg, opts) {
    self.loggerUtil.info(traceId, 'pomelo-rpc', true, serverId, __filename, 'send', 'get corresponding mailbox and try to send message');
    var mailbox = self.mailboxes[serverId];
    if(!mailbox) {
      var args = [new Error('can not find mailbox with id:' + serverId)];
      doFilter(traceId, self, serverId, msg, opts, self.afters, 0, 'after', function() {
        utils.applyCallback(cb, args);
      });
      return;
    }

    mailbox.send(traceId, msg, opts, function() {
  ...
  doFilter(traceId, self, serverId, msg, opts, this.befores, 0, 'before', send);
};
 ...
 */
var doFilter = function(traceId, station, serverId, msg, opts, filters, index, operate, cb) {
  station.loggerUtil.info(traceId, 'pomelo-rpc', true, serverId, __filename, 'doFilter', 'do ' + operate + ' filter ' + index);
  if(index  >= filters.length) {
    utils.invokeCallback(cb, traceId, serverId, msg, opts);
    return;
  }
  ...
  if(typeof filter === 'function') {
    filter(serverId, msg, opts, function(serverId, msg, opts) {
      index++;
      doFilter(traceId, station, serverId, msg, opts, filters, index, operate, cb);
    });
    return;
...
var lazyConnect = function(traceId, station, serverId, factory) {
  station.loggerUtil.info(traceId, 'pomelo-rpc', true, serverId, __filename, 'lazyConnect', 'create mailbox and try to connect to remote server');
  ...
  mailbox.connect(traceId, function(err) {
    if(err) {
    ...
    flushPending(traceId, station, serverId);
  });
  return true;
};

var addToPending = function(traceId, station, serverId, args) {
  station.loggerUtil.info(traceId, 'pomelo-rpc', true, serverId, __filename, 'addToPending', 'add pending requests to pending queue');
...
var flushPending = function(traceId, station, serverId) {
  station.loggerUtil.info(traceId, 'pomelo-rpc', true, serverId, __filename, 'flushPending', 'flush pending requests to dispatch method');
  var pending = station.pendings[serverId];
