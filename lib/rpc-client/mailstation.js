...
pro.dispatch = function(tracer, serverId, msg, opts, cb) {
  tracer.info('pomelo-rpc', true, serverId, __filename, 'dispatch', 'dispatch rpc message to the mailbox');
  if(this.state !== STATE_STARTED) {
  ...
  var send = function(tracer, serverId, msg, opts) {
    tracer.info('pomelo-rpc', true, serverId, __filename, 'send', 'get corresponding mailbox and try to send message');
...
var doFilter = function(tracer, serverId, msg, opts, filters, index, operate, cb) {
  tracer.info('pomelo-rpc', true, serverId, __filename, 'doFilter', 'do ' + operate + ' filter ' + index);
...
var lazyConnect = function(tracer, station, serverId, factory) {
  tracer.info('pomelo-rpc', true, serverId, __filename, 'lazyConnect', 'create mailbox and try to connect to remote server');
...
var addToPending = function(tracer, station, serverId, args) {
  tracer.info('pomelo-rpc', true, serverId, __filename, 'addToPending', 'add pending requests to pending queue');
...
var flushPending = function(tracer, station, serverId) {
  tracer.info('pomelo-rpc', true, serverId, __filename, 'flushPending', 'flush pending requests to dispatch method');
  var pending = station.pendings[serverId];
