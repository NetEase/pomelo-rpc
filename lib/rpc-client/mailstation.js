...
pro.removeServer = function(id) {
  if(!!this.servers[id]) {
    var type = this.servers[id].serverType;
    var servers = this.serversMap[type];
    var index = servers.indexOf(id);
    if(index >= 0) {
...
pro.dispatch = function(tracer, serverId, msg, opts, cb) {
  ...
  if(this.state !== STATE_STARTED) {
    ...
    utils.invokeCallback(cb);
    throw new Error('[pomelo-rpc] client is not running now.');
  }
  ...
  if(!mailbox) {
    ...
    if(!lazyConnect(tracer, this, serverId, this.mailboxFactory, cb)) {
      ...
      utils.invokeCallback(cb);
      throw new Error('[pomelo-rpc] fail to connect to remote server:' + serverId);
    }
  ...
  var send = function(tracer, err, serverId, msg, opts) {
    ...
    if(!!err) {
      errorHandler(self, err, serverId, msg, opts, true, cb);
      return;
    }
    if(!mailbox) {
      tracer.error('client', __filename, 'send', 'can not find mailbox with id:' + serverId);
      utils.invokeCallback(cb);
      throw new Error('[pomelo-rpc] could not find mailbox with id:' + serverId);
    }
    mailbox.send(tracer, msg, opts, function() {
      ...
      doFilter(tracer_send, null, serverId, msg, opts, self.afters, 0, 'after', function(tracer, err, serverId, msg, opts) {
        if(!!err) {
          errorHandler(self, err, serverId, msg, opts, false, cb);
        }
...
var lazyConnect = function(tracer, station, serverId, factory, cb) {
  ...
  mailbox.connect(tracer, function(err) {
    ...
    flushPending(tracer, station, serverId, cb);
  });
  return true;
};
...
var flushPending = function(tracer, station, serverId, cb) {
  ...
  if(!mailbox) {
    ...
    utils.invokeCallback(cb);
    throw new Error('[pomelo-rpc] fail to flush pending messages for empty mailbox: ' + serverId);
  }
...
var errorHandler = function(station, err, serverId, msg, opts, flag, cb) {
  if(!!station.handleError) {
    station.handleError(err, serverId, msg, opts);
  } else {
    utils.invokeCallback(cb);
    throw new Error('[pomelo-rpc] rpc filter error with serverId: ' + serverId);
  }
};
