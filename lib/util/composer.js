 ...
 *
 * @param  {uint8}  type message type that would be composed.
 * @param  {String|Buffer}  data data that would be composed.
 * @param  {uint32}  msg id that would be composed.
 * @return {Buffer}        compose result in Buffer.
 */
pro.compose = function(type, data, id) {
  ...
  if(!!data){//id temperary no need 
    ...
    var lsize = calLengthSize(dataLength);
    var buf = new Buffer(lsize + dataLength);
