...
// promise support,
Utils.promisifyByCallback = function(cb) {
  if (typeof cb == 'function') {
    return cb;
  }
  // if the params cb is null,  return a proxy callback function;
  else {
    var proxyCB = function (err) {
      if (err) {
        proxyCB._reject(err);
      }
      else {
        proxyCB._resovle(Array.prototype.slice.apply(arguments, 1));
      }
    };

		proxyCB.promise = new Promise(function (resovle, reject) {
			proxyCB._resovle = resovle;
			proxyCB._reject = reject;
		});

		proxyCB.promise = promise;
		return proxyCB;
	}
};
